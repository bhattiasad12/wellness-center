                    ...
                    <div class="card card-docs mb-2">
                        <div class="p-10">
                            ...
                            <h1 class="anchor fw-bolder mb-5" id="zero-configuration">
                                <a href="javascript:;"></a>Packs Listing
                            </h1>
                            <!--begin::Notice-->
                        ...
                        <div class="card-header border-0">
                            ...
                            <div class="card-toolbar">
                                ...
                                <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                                    ...
                                    <button type="button" class="btn btn-primary" onclick="addPack()">
                            ...
                            <div class="py-5">
                                <table class="kt_datatable_example_1 table table-row-bordered gy-5">
                                    ...
                                    <tbody class="fw-bold text-gray-600">
                                        @for ($i = 0; $i < count($pack); $i++) @php $a=$i; $a++; @endphp <tr>
                                            <td>{{ $a }}</td>
                                            <td>{{ ucwords($pack[$i]->pack_name) }}</td>
                                            <td>{{ ucwords($pack[$i]->services_name) }}</td>
                                            <td>{{ ucwords($pack[$i]->session) }}</td>
                                            <td>{{ ucwords($pack[$i]->pack_price) }}</td>
                                            <td>
                                                <button type="button" class="btn btn-icon btn-sm btn-color-gray-400 btn-active-icon-primary me-2" onclick="editPack('{{ $pack[$i]->id }}')">
                                                ...
                                                <form style="display: inline-block" method="POST" action="{{ route('pack.destroy', $pack[$i]->id) }}">
                                                    @method('DELETE')
                                                    @csrf
                                                    <button type="submit" class="btn btn-icon btn-sm btn-color-gray-400 btn-active-icon-danger me-2" data-bs-toggle="tooltip" data-bs-original-title="Delete Pack">
...
<script>
    function addPack() {
        $.ajax({
            ...
            success: function(result) {
                $('#myModalLgHeading').html('Add Pack');
    ...
    function editPack(id) {
        url = "{{route('pack.edit',':id')}}";
        url = url.replace(':id', id);
        $.ajax({
            type: 'GET',
            url: url,
            success: function(result) {
                $('#myModalLgHeading').html('Edit Pack');
                $('#modalBodyLarge').html(result);
                $('#myModalLg').modal('show');
            }
        });
